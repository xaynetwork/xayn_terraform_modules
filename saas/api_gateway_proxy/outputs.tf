...
output "api_cloudfront_domain_name" {
  ...
  value       = try(aws_api_gateway_domain_name.domain[count.index].cloudfront_domain_name, null)
}

output "api_cloudfront_zone_id" {
  description = "The Cloudfront zone ID from the API"
  value       = try(aws_api_gateway_domain_name.domain[count.index].cloudfront_zone_id, null)
}
