locals {
  ...
  service_name   = "${local.container_name}-${var.name}"
      ...
      secrets = [
        {
          ...
          value = var.elasticsearch_password_ssm_parameter_arn
        },
        {
          name  = "XAYN_WEB_API__STORAGE__POSTGRES__PASSWORD"
          value = var.postgres_password_ssm_parameter_arn
        }
      ]
    }
  }
...
resource "aws_lb_listener_rule" "service" {
  ...
  condition {
    path_pattern {
      values = var.alb.rules[count.index]
    }
  }
...
module "alarms" {
  ...
  log_group_name = module.service.container_definitions.cloudwatch_log_group_name
